...
class Signer(object):
    ...
    def sign(self, data):
        ...
        sign_digest = sha1(string_data.encode('utf-8')).hexdigest()
    ...
    def validate(self, data):
        ...
        data_copy = data.copy()
        signature = data_copy['signature']
        data_copy.pop('signature')
        signature = signature.replace(' ', '+')
        signature = base64.b64decode(signature)

        if 'fileContent' in data_copy and data_copy['fileContent']:
            file_content = data_copy['fileContent'].replace(' ', '+')
            data_copy.update(fileContent=file_content)
        stringData = self.simple_urlencode(data_copy)
        digest = sha1(stringData.encode('utf-8')).hexdigest()
